...
# Function to get pie chart names based on class counts
def get_pie_chart_names(data):
    class_counts = data['class'].value_counts().sort_values(ascending=False)
    pie_chart_names = list(class_counts.index)  # Get class labels
    return pie_chart_names
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(filtered_df, values='class', 
                     names='Launch Site', 
                     title='Total Launch Outcome by Site')
    else:
        filtered_df = filtered_df[filtered_df['Launch Site'] == entered_site]
        if filtered_df.empty:
            fig = go.Figure(data=[go.Pie(labels=['No Data'], values=[1])])
            fig.update_layout(title_text=f'No Launches for {entered_site}')
        else:
...
def get_scatter_plot(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        ...
        if filtered_df.empty:
            ...
            fig.update_layout(title_text=f"No data available for {entered_site}")
        else:
            fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", 
                             color="Booster Version Category",
